...
#define kTextColor [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:1]
...
#define kTextSelectColor [UIColor colorWithRed:244/255.0 green:198/255.0 blue:43/255.0 alpha:1]
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_currentSelectedMenudIndex==1) {
        if (indexPath.row==0) {
            ...
            if (!cell) {
                ...
                cell.swit.on=self.isOpen;
                cell.block=^(NSInteger status){
                //认证状态
                    [self.valueDict setObject:[NSString stringWithFormat:@"%lu",status] forKey:@"filterCertification"];
        ...
        if (indexPath.row==1) {
            skillTableViewCell*cell=[tableView dequeueReusableCellWithIdentifier:@"skill"];
            if (!cell) {
                cell=[[[NSBundle mainBundle]loadNibNamed:@"skillTableViewCell" owner:nil options:nil]lastObject];
                                cell.selectionStyle=UITableViewCellSelectionStyleNone;
                cell.block=^(NSMutableArray*array){
                    NSString*tempString;
                    for (NSInteger i=0; i<array.count; i++) {
                        ...
                        if (i==0) {
                            ...
                            tempString=[NSString stringWithFormat:@"%@,%@",tempString,temp];
                        }
                    }
                    if (tempString) {
            
                    [_valueDict setObject:tempString forKey:@"filterSkill"];
        ...
        if (indexPath.row==2) {
            ...
            if (!cell) {
                cell=[[[NSBundle mainBundle]loadNibNamed:@"secondTableViewCell" owner:nil options:nil]lastObject];
                               cell.selectionStyle=UITableViewCellSelectionStyleNone;
                cell.block=^(NSString*function){
                    [_valueDict setObject:function forKey:@"filterWorkYearArea"];
                
                };
            }
            cell.selectedDic=self.selectDict;
            cell.exBlock=^{
                [_valueDict removeObjectForKey:@"filterWorkYearArea"];
        ...
        if (indexPath.row==3) {
            fourthTableViewCell*cell=[tableView dequeueReusableCellWithIdentifier:@"four"];
            if (!cell) {
                cell=[[[NSBundle mainBundle]loadNibNamed:@"fourthTableViewCell" owner:nil options:nil]lastObject];
                cell.selectionStyle=UITableViewCellSelectionStyleNone;
                cell.block=^(NSString*function){
                    [_valueDict setObject:function forKey:@"filterAgeArea"];
                };
            }
            cell.faileBlock=^{
                if ([_valueDict objectForKey:@"filterAgeArea"]) {
                    [_valueDict removeObjectForKey:@"filterAgeArea"];
                }
    ...
    //_________________________________________________________________________
            //萌萌的分割线
            if ([cell.textLabel.text isEqualToString:@"待遇"]==YES) {
...
-(UIView*)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    UIButton*resetButton=[[UIButton alloc]initWithFrame:CGRectMake(10, 10, 100, 38)];
    UIButton*confirmButton=[[UIButton alloc]initWithFrame:CGRectMake(130, 10, SCREEN_WIDTH-130-10, 38)];
    [resetButton setTitle:@"重置" forState:UIControlStateNormal];
    [resetButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    resetButton.layer.cornerRadius=4;
    resetButton.backgroundColor=COLOR(244, 199, 44, 1);
    resetButton.titleLabel.font=[UIFont boldSystemFontOfSize:20];
    [confirmButton setTitle:@"确定" forState:UIControlStateNormal];
    confirmButton.layer.cornerRadius=4;
    [confirmButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    confirmButton.backgroundColor=COLOR(22, 167, 233, 1);
    confirmButton.titleLabel.font=[UIFont boldSystemFontOfSize:20];
    [resetButton addTarget: self action:@selector(reset) forControlEvents:UIControlEventTouchUpInside];
    [confirmButton addTarget:self action:@selector(referData) forControlEvents:UIControlEventTouchUpInside];
    view.backgroundColor=COLOR(246, 246, 246, 1);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_currentSelectedMenudIndex==1) {
        if (_rankblock) {
            _rankblock(indexPath.row);
        }
    }else{
    
        if (self.thirdBlock) {
            self.thirdBlock(indexPath);
        }
    }
}
...
- (BOOL )confiMenuWithSelectRow:(NSInteger)row {
    ...
    if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:_currentSelectedMenudRow column:_currentSelectedMenudIndex]> 0) {
        ...
        [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
            ...
            //___________不限制——————————————————————————————————————————————
            if ([title.string isEqualToString:@"不限制"]==YES) {
                title.string=@"待遇";
            }
